...
void updateTheCustomer(fileNames* names)                                                                 //update-m
{
	...
	{
		printf("\n1)Имя Фамилия.\n2)Телефон.\n");
		int number = readInt("введите номер того, что хотите обновить");
		switch (number)
		{
		case 1:
		{
		...
		case 2:
		{
			person.phone = readInt("новый телефон");
			sum++;
			continue;
		}
		break;
...
void deleteTour(fileNames* names)//del-s
{
	...
	printIndexes(indexes, size, "туры");
	int pKey = readInt("pKey тура");
	int i = getIndexByPKey(indexes, size, pKey);
	if (i == -1)
	{
